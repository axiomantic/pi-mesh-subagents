...
/**
 * pi-mesh-subagents installer
 * 
 * Usage:
 *   npx pi-mesh-subagents          # Install to ~/.pi/agent/extensions/subagent
 *   npx pi-mesh-subagents --remove # Remove the extension
 */
...
const REPO_URL = "https://github.com/axiomantic/pi-mesh-subagents.git";
...
if (isHelp) {
	console.log(`
pi-mesh-subagents - Pi extension for delegating tasks to subagents

Usage:
  npx pi-mesh-subagents          Install the extension
  npx pi-mesh-subagents --remove Remove the extension
  npx pi-mesh-subagents --help   Show this help
...
if (isRemove) {
	if (fs.existsSync(EXTENSION_DIR)) {
		...
		console.log("pi-mesh-subagents removed");
	} else {
		console.log("pi-mesh-subagents is not installed");
	}
	process.exit(0);
}

// Install
console.log("Installing pi-mesh-subagents...\n");
...
if (fs.existsSync(EXTENSION_DIR)) {
	...
	if (isGitRepo) {
		...
		try {
			...
			console.log("\npi-mesh-subagents updated");
		} catch (err) {
			console.error("Failed to update. Try removing and reinstalling:");
			console.error("  npx pi-mesh-subagents --remove && npx pi-mesh-subagents");
			process.exit(1);
		}
	} else {
		console.log(`Directory exists but is not a git repo: ${EXTENSION_DIR}`);
		console.log("Remove it first with: npx pi-mesh-subagents --remove");
		process.exit(1);
	}
} else {
	// Fresh install

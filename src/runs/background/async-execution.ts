...
function spawnRunner(cfg: object, suffix: string, cwd: string): { pid?: number; error?: string } {
	...
	proc.on("error", (error) => {
		console.error(`[pi-mesh-subagents] async spawn failed: ${error.message}`);
	});
...
const UNAVAILABLE_SUBAGENT_SKILL_ERROR = "Skills not found: pi-mesh-subagents";
...
export function executeAsyncChain(
	...
	const buildSeqStep = (s: SequentialStep, sessionFile?: string, behaviorCwd?: string, progressPrecreated = false, resolvedBehavior?: ResolvedStepBehavior) => {
		...
		if (missingSkills.includes("pi-mesh-subagents")) throw new UnavailableSubagentSkillError(UNAVAILABLE_SUBAGENT_SKILL_ERROR);
...
export function executeAsyncSingle(
	...
	if (missingSkills.includes("pi-mesh-subagents")) return formatAsyncStartError("single", UNAVAILABLE_SUBAGENT_SKILL_ERROR);

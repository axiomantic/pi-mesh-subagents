...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("fails foreground runs on explicit unavailable pi-mesh-subagents skill requests without spawning", async () => {
		const agents = [makeAgent("worker")];

		const result = await runSync(tempDir, agents, "worker", "Task", { skills: ["pi-mesh-subagents"] });

		assert.equal(result.exitCode, 1);
		assert.equal(result.error, "Skills not found: pi-mesh-subagents");
		assert.equal(mockPi.callCount(), 0);
	});

	it("fails foreground runs when an agent default requests pi-mesh-subagents skill", async () => {
		const agents = [makeAgent("worker", { skills: ["pi-mesh-subagents"] })];
